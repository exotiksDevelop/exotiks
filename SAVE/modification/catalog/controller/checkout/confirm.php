		...
		if (!$redirect) {
			...
			foreach ($order_data['totals'] as $key => $value) {
                ...
                if (isset($this->session->data['shipping_method']['text']) and !empty($this->session->data['shipping_method']['text'])) {
                    foreach ($order_data['totals'] as $k => $v) {
                        if ($v['code'] == 'shipping') {
                            $order_data['totals'][$k]['text'] = $this->session->data['shipping_method']['text'];
                        }
                    }
                }
          ...
          $payment_module = $parts[0];
			...
			$data['payment'] = $this->load->controller('payment/' . $payment_module);
		} else {
