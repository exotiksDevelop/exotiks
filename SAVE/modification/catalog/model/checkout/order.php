...
class ModelCheckoutOrder extends Model {
  ...
  public function rememberCdek($order_id, $data = array())
  {
    if(!count($data))
    {
      $data['sdek_city'] = '';
      ...
      if(isset($this->session->data['sdek']['pvz']))
        $data['sdek_pvz'] = $this->session->data['sdek']['pvz'];

      if(isset($this->session->data['sdek']['city']))
        $data['sdek_city'] = $this->session->data['sdek']['city'];

      if(isset($this->session->data['sdek']['pvzinfo']))
        $data['sdek_pvzinfo'] = $this->session->data['sdek']['pvzinfo'];
    }

    $sql = "INSERT INTO `" . DB_PREFIX . "order_to_sdek` (`order_to_sdek_id`, `order_id`, `cityId`, `pvz_code`)
    VALUES (NULL, '".$order_id."', '".$data['sdek_city']."','".$data['sdek_pvz']."') ON DUPLICATE KEY UPDATE
    cityId = '".$data['sdek_city']."', pvz_code='".$data['sdek_pvz']."'";

    $this->db->query($sql);

    if(isset($data['sdek_pvzinfo']) && $data['sdek_pvzinfo']) {
      ...
      if($comment)
        $newComment = $comment . "\n" . '[CDEK]Выбранный ПВЗ: ' . $data['sdek_pvzinfo'] . '[/CDEK]';
      else
        $newComment = '[CDEK]Выбранный ПВЗ: ' . $data['sdek_pvzinfo'] . '[/CDEK]';
  ...
  public function clearCdek($order_id)
  {
    $sql = "SELECT * FROM `" . DB_PREFIX . "order_to_sdek` WHERE order_id = '".$order_id."'";
    ...
    if($query->num_rows) {
      ...
      $this->rememberCdek($order_id, $data);
