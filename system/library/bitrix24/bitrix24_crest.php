...
class CRest
{
	...
	protected static function callCurl($arParams)
	{
		...
		if ($arSettings !== false) {
			...
			try {
				...
				if (isset($arParams['this_auth']) && $arParams['this_auth'] != 'Y' && static::TYPE_TRANSPORT == 'xml') {//auth only json support
					$result = $out;
				} else {
	...
	public static function setLog($arData, $type = '')
	{
		...
		file_put_contents(DIR_LOGS . static::$logFile, date("Y-m-d H:i:s - ") . ' ' . print_r($arData, true) . "\r\n", FILE_APPEND);
	}
