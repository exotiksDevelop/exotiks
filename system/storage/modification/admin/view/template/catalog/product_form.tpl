        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          ...
          <div class="tab-content">
                ...
                <div class="tab-pane" id="language<?php echo $language['language_id']; ?>">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
					  ...
					  <input type="hidden" name="product_description[<?php echo $language['language_id']; ?>][tag]" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['tag'] : ''; ?>" id="input-tag<?php echo $language['language_id']; ?>" />
                    </div>
                  </div>
                </div>
                <?php } ?>
            ...
            <div class="tab-pane" id="tab-links">
			  ...
			  <div class="form-group">
                <label class="col-sm-2 control-label" for="input-tagn"><span data-toggle="tooltip" title="<?php echo $help_tagn; ?>"><?php echo $entry_tagn; ?></span></label>
                <div class="col-sm-10">
                  <input type="text" name="tagn" value="" placeholder="<?php echo $entry_tagn; ?>" id="input-tagn" class="form-control" />
                  <div id="product-tagn" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($product_tagns as $product_tag) { ?>
                      <div id="product-tagn<?php echo $product_tag['tag_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_tag['name']; ?>
                        <input type="hidden" name="product_tagn[]" value="<?php echo $product_tag['tag_id']; ?>" />
                      </div>
                    <?php } ?>
                  </div>
                </div>
			...
			<div class="tab-pane" id="tab-tags">
              <div class = "left_img_block">
                <!--<img src="<?php echo $tags_img; ?>" alt="" title="" data-placeholder="<?php echo $placeholder; ?>" />-->
              </div>
              <div class = "right_tags_block">
                <?foreach ($product_categories_tags as $product_category) 
				{ 
					if (count($product_category['tags']) > 0)
					{?>
						<div id="tags-product-category<?php echo $product_category['category_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_category['name']; ?>
							<?php foreach ($product_category['tags'] as $tag) { ?>
							  <div class = "tag_container">
								<input type = "checkbox" <?if (isset($tag['checked']) && ($tag['checked'] == true)) { echo('checked'); }?> data-tag-id="<?=$tag['tag_id']?>" id = "product_tag_<?=$tag['tag_id']?>" name="product_tags[]" value="<?php echo $tag['tag_id']; ?>" />
								<label for = "product_tag_<?=$tag['tag_id']?>"><?=$tag['name']?></label>
								<div class = "clear"></div>
							  </div>
							<?}?>
						</div>
...
// tags
$('input[name=\'tagn\']').autocomplete({
  'source': function(request, response) {
	$.ajax({
	  url: 'index.php?route=catalog/tags/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
	  dataType: 'json',     
	  success: function(json) {
		response($.map(json, function(item) {
		  return {
  ...
  'select': function(item) {
	...
	$('#product-tagn' + item['value']).remove();
	
	$('#product-tagn').append('<div id="product-tagn' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="product_tagn[]" value="' + item['value'] + '" /></div>'); 
  }
});

$('#product-tagn').delegate('.fa-minus-circle', 'click', function() {
  $(this).parent().remove();
});
