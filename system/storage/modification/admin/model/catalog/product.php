...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_tags'])) {
			foreach ($data['product_tags'] as $tag_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_tag SET product_id = '" . (int)$product_id . "', tag_id = '" . (int)$tag_id . "'");
			}
		}
		// end tags
	...
	public function editProduct($product_id, $data) {
		...
		$insert_product_tags = array();
		if (isset($data['product_tagn']) && (count($data['product_tagn']) > 0))
		{
			foreach ($data['product_tagn'] as $cur_tag_id) {
				$sql = "select * from " . DB_PREFIX . "product_to_tag where product_id = ".(int)$product_id." and tag_id = ".(int)$cur_tag_id;
				$q = $this->db->query($sql);
				if (!$q->num_rows){
					$insert_product_tags[$cur_tag_id] = $cur_tag_id;
					$sql = "insert into " . DB_PREFIX . "product_to_tag set product_id = ".(int)$product_id.", tag_id = ".(int)$cur_tag_id;
					$this->db->query($sql);
				}
			}
		}
		
		if (isset($data['product_tags']) && (count($data['product_tags']) > 0))
		{
			foreach ($data['product_tags'] as $cur_tag_id) {
				if (!isset($insert_product_tags[$cur_tag_id]))
				{
					$sql = "select * from " . DB_PREFIX . "product_to_tag where product_id = ".(int)$product_id." and tag_id = ".(int)$cur_tag_id;
					$q = $this->db->query($sql);
					if (!$q->num_rows){
