...
class ControllerCommonMenu extends Controller {
	public function index() {
		$this->load->language('common/menu');
          ...
          $data['cdek_dispatch'] = array('text'=>'Отгрузка в СДЭК', 'link'=> $this->url->link('module/cdek_integrator', 'token=' . $this->session->data['token'], 'SSL'));
          $data['cdek_shipping'] = array('text'=>'Расчет доставки', 'link'=> $this->url->link('shipping/cdek', 'token=' . $this->session->data['token'], 'SSL'));
          $data['cdek_payment'] = array('text'=>'Наложенный платеж', 'link'=> $this->url->link('payment/cod_cdek', 'token=' . $this->session->data['token'], 'SSL'));
          $data['cdek_total'] = array('text'=>'Учитывать в заказе', 'link'=> $this->url->link('total/cod_cdek_total', 'token=' . $this->session->data['token'], 'SSL'));
          $data['cdek_documentation'] = array('text'=>'Документация', 'link'=> 'http://cdek.opencart.ru/documentation/#oc2');
          $data['cdek_help'] = array('text'=>'Поддержка', 'link'=> 'http://helpdesk.opencart.ru');
          $data['cdek_license'] = array('text'=>'Лицензия', 'link'=> $this->url->link('tool/cdektool', 'token=' . $this->session->data['token'], 'SSL'));
        

		// Ozon Seller
    $data['cdl_ozon_seller'] = $this->url->link('module/ozon_seller', 'token=' . $this->session->data['token'], 'SSL');
    $data['cdl_ozon_seller_product'] = $this->url->link('module/ozon_seller/product', 'token=' . $this->session->data['token'], 'SSL');
    $data['cdl_ozon_seller_order'] = $this->url->link('module/ozon_seller/order', 'token=' . $this->session->data['token'], 'SSL');
    $data['cdl_ozon_seller_export'] = $this->url->link('module/ozon_seller/export', 'token=' . $this->session->data['token'], 'SSL');

		if ($this->config->get('ozon_seller_act')) {
			$check_act_ozon = glob(DIR_UPLOAD . date('Y-m-d') . '_act_ozon_seller*.pdf');
			if (!empty($check_act_ozon)) {
				$data['act_ozon'] = '<li><a href="' . $this->url->link('module/ozon_seller/downloadact', 'token=' . $this->session->data['token'], true) . '">Скачать акт</a></li>';
			} else {
				$data['act_ozon'] = '<li><a href="' . $this->url->link('module/ozon_seller', 'token=' . $this->session->data['token'], true) . '">Акт не доступен</a></li>';
			}
		}
      

		//cdl_wildberries
		$data['cdl_wildberries'] = $this->url->link('module/cdl_wildberries', 'token=' . $this->session->data['token'], 'SSL');
		$data['cdl_rima'] = $this->url->link('module/cdl_wildberries/rima', 'token=' . $this->session->data['token'], 'SSL');
		$data['cdl_attributes'] = $this->url->link('module/cdl_wildberries/attributes', 'token=' . $this->session->data['token'], 'SSL');
		$data['cdl_product'] = $this->url->link('module/cdl_wildberries/product', 'token=' . $this->session->data['token'], 'SSL');
		$data['cdl_orders_wb'] = $this->url->link('module/cdl_wildberries/orders', 'token=' . $this->session->data['token'], 'SSL');
		$data['cdl_supplies'] = HTTPS_CATALOG . 'index.php?route=module/cdl_wildberries_supplies/pass&request=0&pass=' . $this->config->get('cdl_wildberries_pass_supplies');
		$data['cdl_statistics'] = HTTPS_CATALOG . 'index.php?route=module/cdl_wildberries_statistics&pass=' . $this->config->get('cdl_wildberries_pass');
...
$data['url_redirect'] = $this->url->link('module/url_redirect', 'token=' . $this->session->data['token'], 'SSL');
...
$data['news'] = $this->url->link('catalog/news', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['product'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'], 'SSL');
				...
				$data['wb_product'] = $this->url->link('catalog/wildberries', 'token=' . $this->session->data['token'], true);
				$data['wb_product_unattach'] = $this->url->link('catalog/wildberries/getUnsorted', 'token=' . $this->session->data['token'], true);
				$data['wb_settings'] = $this->url->link('module/wildberries', 'token=' . $this->session->data['token'], true);
		...
		$data['tags'] = $this->url->link('catalog/tags', 'token=' . $this->session->data['token'], 'SSL');


                    $data['vk_export'] = $this->url->link('extension/vk_export', 'token=' . $this->session->data['token'], 'SSL');
                    $data['vk_export_albums'] = $this->url->link('extension/vk_export/albums', 'token=' . $this->session->data['token'], 'SSL');
                    $data['vk_export_market'] = $this->url->link('extension/vk_export/market', 'token=' . $this->session->data['token'], 'SSL');
                    $data['vk_export_setting'] = $this->url->link('module/vk_export', 'token=' . $this->session->data['token'], 'SSL');
                    $data['vk_export_report'] = $this->url->link('extension/vk_export/report', 'token=' . $this->session->data['token'], 'SSL');

                    $this->language->load('extension/vk_export_header');
                    $data['text_vk_export'] = $this->language->get('text_vk_export');
                    $data['text_vk_export_albums'] = $this->language->get('text_vk_export_albums');
                    $data['text_vk_export_setting'] = $this->language->get('text_vk_export_setting');
                    $data['text_vk_export_cron_report'] = $this->language->get('text_vk_export_cron_report');
                    $data['text_vk_export_market'] = $this->language->get('text_vk_export_market');
