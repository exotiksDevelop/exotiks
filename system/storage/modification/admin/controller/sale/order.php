...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('sale/order/add', 'token=' . $this->session->data['token'], 'SSL');
			...
			$data['exportListOrder'] = $this->url->link('module/b24_apipro/exportListOrder', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach ($results as $result) {
			...
			$status_b24 = !empty($this->model_module_b24_order->getById($result['order_id'])) ? 'Да' : 'Нет';
	...
	public function info() {
		...
		if ($order_info) {
          ...
          $payment_code = $payment_code[0];
			...
			$content = $this->load->controller('payment/' . $payment_code . '/order');

			if ($content) {
				$this->load->language('payment/' . $payment_code);
