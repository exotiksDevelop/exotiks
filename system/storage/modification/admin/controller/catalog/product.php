...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data['text_no_results'] = $this->language->get('text_no_results');
			...
			$data['columncategory'] = $this->language->get('columncategory');
			$data['entrycategory'] = $this->language->get('entrycategory');
		...
		/* Quick Edit Starts */ 
		$data['button_quick_add'] = $this->language->get('button_quick_add');
		$data['button_quick_edit'] = $this->language->get('button_quick_edit');
		$data['quickproduct_edit_action'] = str_replace('&amp;', '&',$this->url->link('catalog/quickproduct', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		$data['quickproduct_add_action'] = str_replace('&amp;', '&',$this->url->link('catalog/quickproduct', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		
		if ($this->user->hasPermission('access', 'catalog/quickproduct')) {
			$data['permission_file'] = true;
		}else{
			$data['permission_file'] = false;
		}
		/* Quick Edit Ends */ 
		...
		$data['footer'] = $this->load->controller('common/footer');
            ...
            foreach($data['products'] as &$arProduct) {
				$b24ProductList = $this->model_module_b24_product->getByid($arProduct['product_id']);
				$b24_product_status = '';
				foreach ($b24ProductList as $b24Product) {
					$b24_product_status .= "<small>Размер: {$b24Product['option']}. ID Б24: {$b24Product['b24_product_id']}. Дата: " . date('d.m.Y', strtotime($b24Product['date_update'])) . "<br></small>";
				}
				$arProduct['b24_product_status'] = $b24_product_status;
              }              
	...
	protected function getForm() {
		...
		$data['help_tagn'] = $this->language->get('help_tagn');
		$data['entry_tagn'] = $this->language->get('entry_tagn');
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			...
			$data['tags_img'] = $this->model_tool_image->resize($this->request->post['image'], 400, 400);
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['image'])) {
			$data['thumb'] = $this->model_tool_image->resize($product_info['image'], 100, 100);
			$data['tags_img'] = $this->model_tool_image->resize($product_info['image'], 400, 400);
		} else {
			$data['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
			$data['tags_img'] = $this->model_tool_image->resize('no_image.png', 400, 400);
		}
		...
		if (isset($this->request->post['product_tagn'])) {
			$tags = $this->request->post['product_tagn'];
		} elseif (isset($this->request->get['product_id'])) {
		...
		$data['product_categories_tags'] = array();

		foreach ($categories as $category_id) {
			$category_info = $this->model_catalog_category->getCategory($category_id);

			if ($category_info) {
				$tags_list = $this->model_catalog_tags->getCategoryTags($category_info['category_id']);
				
				foreach($tags_list as $key_tag => $tag_item)
				{
					if (in_array($tag_item['tag_id'], $tags))
					{
						$tags_list[$key_tag]['checked'] = true;
					}
				}
