...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		$data['delete'] = $this->url->link('customer/customer/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			$data['exportListCustomer'] = $this->url->link('module/b24_apipro/exportListCustomer', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach ($results as $result) {
			...
			$status_b24 = !empty($this->model_module_b24_customer->getById($result['customer_id'])) ? 'Да' : 'Нет';
