<div class="form-group <?php if ($required) { ?>required<?php } ?> row-<?php echo $id ?>">
    <label class="control-label col-12" for="<?php echo $id ?>"><?php echo $label ?></label>
    <div class="col-12">
      ...
      <?php } elseif ($type == 'date') { ?>
        <div class="input-group date">
          ...
          <span class="input-group-btn">
            <button type="button" class="button red"><i class="fa fa-calendar"></i></button>
          </span>
        </div>
      ...
      <div class="input-group time">
        <input class="form-control" type="text" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo $value ?>" placeholder="<?php echo $placeholder ?>" <?php echo $attrs ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
        <span class="input-group-btn">
          <button type="button" class="button red"><i class="fa fa-clock-o"></i></button>
        </span>
      </div>
      <?php } else { ?>
        <?php if ($id == 'shipping_address_city' && $value && $progroman_citymanager->setting('replace_input_city')) { ?>
