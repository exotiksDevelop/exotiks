...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="catalog/view/theme/<?= $theme_name ?>/stylesheet/newdesign.css?v=70" />
  ...
  <?php
    ...
    //$url = explode('?', $url);
    //$url = $url[0];
    ///index.php?route=product/category&path=78_164 
    $is_blocked_url = 'false';
    $right_side_url = explode('path=',$url)[1];    
    if( $right_side_url != '' ){
      if( explode('_',$right_side_url)[1] == '' )  $is_blocked_url = 'true';
    }  
     foreach ($links as $link) { 
      if( (
          (urldecode($url) != '/Декоративно-лиственные растения-купить?product_id=905')&&
          (urldecode($url) != '/index.php?route=product/product&product_id=891')
        )||
        ( $link['rel'] != 'canonical' )
      ){ 
        if( $right_side_url != '' ){ 
            if( $is_blocked_url == 'true' ){ ?>
        ...
        <?php   }
            ...
            if( $link['rel'] != ''){
              if( $link['href'] != 'https://exotiks.ru'.$url ){ ?>
              <link href="<?=$link['href']; ?>" rel="<?=$link['rel']; ?>" />
            <?php  }
            }
            else{
...
<style type="text/css">
    .prmn-cmngr_top {display: inline-block;vertical-align: top;height: 52px;line-height: 52px;margin-left: 36px;}
    .prmn-cmngr_top .prmn-cmngr__city {color: #000;padding: 5px 30px;border: 1px solid #8D8D8D;border-radius: 1em;margin-left: 15px; text-decoration: none;}
    .prmn-cmngr_top  .prmn-cmngr__title-text {font-size: 14px; color: #8D8D8D;}
    .prmn-cmngr_top .fa-map-marker {display: none;}
    .prmn-cmngr_top .prmn-cmngr__confirm {top: 45px; left: 30px;}
    .prmn-cmngr_mobile {display: none;}
    .prmn-cmngr_mobile .prmn-cmngr__city {width: auto;}
    .prmn-cmngr_mobile .prmn-cmngr__title-text {color: #fff;}
...
<body class="<?=$class; ?>">
        ...
        <div class="container">
          ...
          <nav class="head1__burger-nav" id="topNav">
            <ul class="head1__burger-nav-ul">
              ...
              <li class="head1__burger-nav-li"><a href="/index.php?route=newsblog/category&newsblog_path=3">Блог</a></li>
         ...
         <li class="head1__burger-nav-li"><div class="prmn-cmngr prmn-cmngr_mobile" data-confirm="false"><?= $prmn_cmngr ?></div></li>
            </ul>
          </nav>
         <div class="prmn-cmngr prmn-cmngr_top"><?= $prmn_cmngr ?></div>
 ...
 <!--<div class="head1__callme-wrap">
            ...
            <div class="head1__callme-wrap">
            <a href="https://t.me/Exotiks_bot" class="head1__callme">
              <span class="head1__callme-point"></span>
              Нужна консультация?
            </a>
        
<!-- Перезвоним Битрикс 24 -->
          <!--<div class="head1__callme-wrap">
            <a href="#myModal" class="head1__callme" data-toggle="modal" data-target="#myModal">
              <span class="head1__callme-point"></span>
              Нужна консультация
    ...
    <div class="navigation">
      <div class="container">
        <ul class="navigation__list">
          ...
          <li class="navigation__list-item"><a href="/index.php?route=newsblog/category&newsblog_path=3">Блог</a></li>
