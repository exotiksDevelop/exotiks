          ...
          'tax'         => $tax,
			...
			if (isset($this->request->get['limit'])) {
				$url .= '&limit=' . $this->request->get['limit'];
					...
					$this->document->setTc_og($this->load->controller('module/microdatapro/tc_og', $data_mpd));
					$this->document->setTc_og_prefix($this->load->controller('module/microdatapro/tc_og_prefix'));
					$data['microdatapro'] = $this->load->controller('module/microdatapro/category', $data_mpd);
			...
			if ($this->config->get('newtags_category')) {
				$this->load->model('catalog/tags');

				$tags = $this->model_catalog_tags->getCategoryTags($category_id);
				...
				if ($tags) {
					foreach ($tags as $tag) { 
						...
						if ($tag['category_id']) {
							...
							$href = $this->url->link('product/category', 'path=' . $path . '&tag_id=' . $tag['tag_id']);
						}
