...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_tag_id'])) {
			$sql .= " AND p.product_id in (select product_id from  ". DB_PREFIX . "product_to_tag where tag_id = " . (int)$data['filter_tag_id'] . ")";
		}
		...
		$product_data = array();
			...
			if (isset($sql) && $this->config->get('sortslimits_hide')){
    			if ($this->config->get('sortslimits_stock_status') == '0'){
					$sql = str_replace("p.status = '1'", "p.status = '1' AND p.quantity > 0", $sql);
				}else{
					$sql = str_replace("p.status = '1'", "p.status = '1' AND p.stock_status_id <> " . $this->config->get('sortslimits_stock_status'), $sql);
				}
    		}else if (isset($sql) && $this->config->get('sortslimits_in_stock') && (strpos($sql, "ps.product_id") || (strpos($sql, "p.product_id")))){
    			if ($this->config->get('sortslimits_stock_status') == '0'){
					$sql = str_replace('ORDER BY', 'ORDER BY (p.quantity > 0) DESC, ', $sql);
				}else{
					$sql = str_replace('ORDER BY', 'ORDER BY (p.stock_status_id <> '.$this->config->get('sortslimits_stock_status').') DESC, ', $sql);
				}
    		}
