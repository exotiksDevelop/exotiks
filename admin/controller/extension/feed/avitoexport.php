<?php
class ControllerExtensionFeedAvitoexport extends Controller
{
	private $error = array();
    ...
    {
        $this->load->language('extension/feed/avitoexport');
        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('setting/setting');
		$this->load->model('extension/feed/avitoexport');
		...
		$data['feed']  	 	  = HTTP_CATALOG . 'index.php?route=extension/feed/avitoexport';
        $data['regions']  	  = $this->model_extension_feed_avitoexport->getRegions();	
		...
		$data['categories']     = $this->model_extension_feed_avitoexport->getCategories();
		$data['subcategories']  = array();

		foreach($data['categories'] as $category){
			$data['subcategories'][$category['category_id']] = $this->model_extension_feed_avitoexport->getCategories($category['category_id']);
		}
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('extension/feed/avitoexport', 'token=' . $this->session->data['token'], true),
            'separator' => ' :: '
        );
        ...
        if (isset($this->request->post['avitoexport_location_city'])) {
			...
			$data['cities'] = $this->model_extension_feed_avitoexport->getCities($data['avitoexport_location_region']);
        ...
        if (isset($this->request->post['avitoexport_location_district'])) {
			...
			$data['city_child'] = $this->model_extension_feed_avitoexport->getCChilds($data['avitoexport_location_city']);
        } else {
        ...
        if (isset($this->request->post['avitoexport_location_subway'])) {
			...
			$data['city_child'] = $this->model_extension_feed_avitoexport->getCChilds($data['avitoexport_location_region']);			
		} else {
            $data['avitoexport_location_subway'] = $this->config->get('avitoexport_location_subway');
			$data['city_child'] = $this->model_extension_feed_avitoexport->getCChilds($data['avitoexport_location_city']);			
        }
        ...
        $data['action'] = $this->url->link('extension/feed/avitoexport', 'token=' . $this->session->data['token'], true);
        $data['getLocation'] = $this->url->link('extension/feed/avitoexport/getLocation', 'token=' . $this->session->data['token'], true);
		$data['getTypeFromCategory'] = $this->url->link('extension/feed/avitoexport/getTypeFromCategory', 'token=' . $this->session->data['token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/feed/avitoexport', $data));
    }
    ...
    public function getLocation(){
		...
		switch ($request){
			case "region": 
				...
				$cities = ($this->model_extension_feed_avitoexport->getCities($regionId));
				$response =<<<HTML
...
HTML;
			...
			case "city":
				...
				$cc = ($this->model_extension_feed_avitoexport->getCChilds($cityId));
...
HTML;
	...
	public function install(){
		$this->load->model('extension/feed/avitoexport');
		$creatingTables = $this->model_extension_feed_avitoexport->createTable();
		if($creatingTables){
						...
						if(count ($city) != 0 ){
							if(isset($city->District)){
								foreach($city->District as $d){
									$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$city['Id'],$city['Name'],$d['Id'],'District',$d['Name']);
									if(!$flag) {
							...
							if(isset($city->Subway)){
								foreach($city->Subway as $s){
									$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$city['Id'],$city['Name'],$s['Id'],'Subway',$s['Name']);
									if(!$flag) {
										echo $error_text; exit;
									}
								}
							...
							if(isset($city->DirectionRoad)){
								foreach($city->DirectionRoad as $dr){
									$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$city['Id'],$city['Name'],$dr['Id'],'DirectionRoad',$dr['Name']);
									if(!$flag) {
										echo $error_text; exit;
									}
								}
							...
							$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$city['Id'],$city['Name'],-1,'none','none');
							if(!$flag) {
								echo $error_text; exit;
							}
						}
					...
					if(count ($region) != 0 ){
						if(isset($region->District)){
							foreach($region->District as $d){
								$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$region["Id"],'except',$d['Id'],'District',$d['Name']);
								if(!$flag) {
									echo $error_text; exit;
								}
							}
						...
						if(isset($region->Subway)){
							foreach($region->Subway as $s){
								$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$region["Id"],'except',$s['Id'],'Subway',$s['Name']);
								if(!$flag) {
									echo $error_text; exit;
								}
							}
						...
						if(isset($region->DirectionRoad)){
							foreach($region->DirectionRoad as $dr){
								$flag = $this->model_extension_feed_avitoexport->put($region["Name"],$region["Id"],$region["Id"],'except',$dr['Id'],'DirectionRoad',$dr['Name']);
								if(!$flag) {
									echo $error_text; exit;
								}
							}
						...
						echo "Type: undefined | City : undefined | Region : " . $region["Name"] . " </br>"; 
					}
				}
			}
		} else {
	...
	public function uninstall(){
		$this->load->model('extension/feed/avitoexport');
		$this->model_extension_feed_avitoexport->deleteTable();
    ...
    {
        if (!$this->user->hasPermission('modify', 'extension/feed/avitoexport')) {
		...
		} /* elseif(!in_array($this->request->post['avitoexport_location_region'],array("637640","653240")) && empty($this->request->post['avitoexport_location_city'])){
			$this->error['error_location_city'] = $this->language->get('error_location');
		} elseif(in_array($this->request->post['avitoexport_location_region'],array("637640","653240")) && empty($this->request->post['avitoexport_location_subway'])) {
			$this->error['error_location_subway'] = $this->language->get('error_location');
		}*/
