      ...
      <div class="panel-body">
        <form action="" method="post" enctype="multipart/form-data" id="form-album" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10"><?php echo (($category['path']) ? $category['path'] . ' &gt; ' : '') . $category['name']; ?></div>
          </div>
              ...
              <?php foreach ($vk_market_categories as $mcats) { ?>
                  <optgroup label="<?php echo $mcats['name']; ?>">
                    <?php foreach ($mcats['childs'] as $mcat) { ?>
                      <option value="<?php echo $mcat->id; ?>"<?php echo (($mcat->id == $market['vk_market_category_id']) ? ' selected="selected"' : ''); ?>><?php echo $mcat->name; ?></option>
                    <?php } ?>
                  </optgroup>
              <?php } ?>
              </select>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" name="vk_market_album_id" placeholder="ссылка на подборку ВК" class="form-control" value="<?php echo htmlspecialchars($market['vk_market_album_id']); ?>"/>
            </div>
          </div>
