	...
	class ControllerExtensionFeedAvitoexport extends Controller {
   		public function index() {			   
			if ($this->config->get('avitoexport_status')) {
				...
				$region = $this->model_extension_feed_avitoexport->getRegion($this->config->get('avitoexport_location_region'));
				if($this->config->get('avitoexport_location_city')){
					$city = $this->model_extension_feed_avitoexport->getCity($this->config->get('avitoexport_location_city'));
				}
				if($this->config->get('avitoexport_location_district')){
					$district = $this->model_extension_feed_avitoexport->getCityChild($this->config->get('avitoexport_location_district'));
				} elseif($this->config->get('avitoexport_location_subway')){
					$subway = $this->model_extension_feed_avitoexport->getCityChild($this->config->get('avitoexport_location_subway'));
				}
					...
					foreach ($productsByCat as $key => $products) {
						...
						foreach ($products as $p) {
							...
							$output .= '<Ad>';
								...
								if ($category == 'Детская одежда и обувь' || $category == 'Товары для детей и игрушки') {
								...
								if(isset($p['image'])&&$p['image']!=''){
									$output .= '<Images>';
									$output .= '<Image url="' . HTTPS_SERVER . 'image/' . $p['image'] . '" />';
									$Images = $this->model_catalog_product->getProductImages($p['product_id']);
									if(count($Images) > 0){
										foreach ($Images as $key => $image) {
											if($key > 9) break;
											$output .= '<Image url="' . HTTPS_SERVER . 'image/' . $image['image'] . '" />';
										}
									}
									$output .= '</Images>';
								}
   		...
   		private function getProduts() {
			...
			if($this->config->get('avitoexport_dependence_id_from')){
				...
				foreach ($categories as $key => $category) {
					array_push($tpm,$this->model_extension_feed_avitoexport->getProductsByCategoryId($category,!empty($stock)));
				}
			}
