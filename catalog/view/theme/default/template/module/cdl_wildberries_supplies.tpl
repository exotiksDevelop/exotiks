    ...
    <div class="content">
      <div class="panel">
        ...
        <?php if (!empty($supplies)) { ?>
          <button type="button" class="btn btn-primary btn-lg btn-block add-to-supplie"><?php echo $text_add_to_supplies; ?></button>
          <a href="<?php echo $url_print_supplie; ?>" class="btn btn-primary btn-lg btn-block" target="_blank"><?php echo $text_print; ?></a>
          <hr>
  ...
  <script>
    ...
    $(document).on('click', '.add-to-supplie', function() {
      data = $('#wb_order').serialize();
    	$.ajax({
    		url: '<?php echo $url_add_supplie; ?>&' + data + '&supplie_id=<?php echo $supplies; ?>',
    		success: function() {
    			location.reload();
    		},
