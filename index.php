...
//REDIRECTOR START
if($config->get('redirector_status')){
$url_to = $db->query("SELECT `url_to` FROM " . DB_PREFIX . "redirector WHERE status = 1 AND url_to != '' AND url_from = '" . $db->escape(str_replace("&","&amp;",str_replace("&amp;", "&", substr(urldecode($request->server["REQUEST_URI"]), 1))))  . "' ORDER BY redirect_id DESC LIMIT 1");
if(isset($url_to->row["url_to"]) && $url_to->row["url_to"]){header("HTTP/1.1 301 Moved Permanently");header("Location: " . HTTPS_SERVER . $url_to->row["url_to"]);exit;}
}
//REDIRECTOR END
...
$loader->controller('extension/module/progroman/citymanager/startup');
